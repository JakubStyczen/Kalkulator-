...
if __name__ == '__main__':
    try:
        operators = {'1': [add, "Arg1 + Arg2"], '2': [sub, "Arg1 - Arg2"], '3': [mul, "Arg1 * Arg2"],
                     '4': [div, "Arg1 / Arg2"], '5': [pot, "Arg1 ^ Arg2"],
                     '6': [nth_root, "Arg1 liczba pierwiastkowana, Arg2 st. pierwiastka"], '7': [logarytm,
        ...
        while True:
            ...
            operator = validatioInputString('Wybierz dzialanie +(1) -(2) *(3) /(4) ^(5) \u221A(6) log(7): ',
                                            operators.keys())
            print()
            ...
            if store.size_of() > 0:
                ...
                if prev == 'Y':
                    for idx, sol in enumerate(store):
                        ...
                        printForm(sol)
                    ans1 = int(
                        validatioInputString('Podaj numer wyniku do 1 argumentu lub 0 jeżeli nie chcesz nic wpisywać: ',
                                             [str(x) for x in range(store.size_of() + 1)]))
                    ans2 = int(
                        validatioInputString('Podaj numer wyniku do 2 argumentu lub 0 jeżeli nie chcesz nic wpisywać: ',
                                             [str(x) for x in range(store.size_of() + 1)]))
